...
% linthresh sets the linear region around zero — below this the scale
% is linear, above it logarithmic. 0.05 keeps small differences visible
% without exaggerating numerical noise.
linthresh = 0.05;
...
% Use pcolor with edge coordinates so cells are bounded exactly by
% the tick locations. Pad data with one extra row+col (pcolor drops
% the last row/col).
...
% Build colour bar with ticks at meaningful real-data values, then
% transform their positions through symlog to place them correctly.
...
cb.Label.String = 'Range bracket advantage ($\lambda$)';
...
% Operational boundary lines (from paper §3.4) — capture handles for legend
h_vlow = yline(velocity_lower_bound, 'k:',  'LineWidth', 1.5);
h_vhi  = yline(velocity_upper_bound, 'k-.', 'LineWidth', 1.5);

% Spatial aliasing band: side-length criterion delta/lambda = 0.5 is the
% naive bound, but for an AVS the relevant spacing is the diagonal
% between the outermost sensors (delta*sqrt(2) for a square layout),
% so aliasing onsets at delta/lambda = 0.5/sqrt(2) ~ 0.354.
alias_diagonal = 0.5 / sqrt(2);

% Hatched overlay marking the aliasing-onset band. Drawn with line
% objects (not patch) so it sits cleanly above the pcolor surface
% without the z-order issues patch + pcolor cause.
xl = xlim;
...
h_alias_d = yline(alias_diagonal,  'k--', 'LineWidth', 1.5);
h_alias_s = yline(alias_threshold, 'k--', 'LineWidth', 1.5);
hold off;

xlabel('Source distance $r/\lambda$', 'FontSize', fs_lab);
ylabel('Array spacing $\delta/\lambda$', 'FontSize', fs_lab);

% Ticks on cell edges, labels on test values only
[x_tick_pos, x_tick_lab] = edge_ticks(distance_lambdas, x_edges);
[y_tick_pos, y_tick_lab] = edge_ticks(delta_lambdas,    y_edges);
set(gca, 'FontName', fn, 'FontSize', fs_ax, 'LineWidth', 1.0, ...
    'Box', 'on', 'TickLabelInterpreter', 'latex', ...
    'XTick', x_tick_pos, 'XTickLabel', x_tick_lab, ...
    'YTick', y_tick_pos, 'YTickLabel', y_tick_lab, ...
    'TickDir', 'out', 'TickLength', [0.012 0.012]);
xlim([x_edges(1), x_edges(end)]);
ylim([y_edges(1), y_edges(end)]);

% Legend below the map. Use an invisible patch as the proxy for the
% aliasing band so the legend swatch reads as a filled rectangle
% rather than a single short hatch line.
h_band_proxy = patch(NaN, NaN, [0.85 0.85 0.85], ...
    'EdgeColor', [0.3 0.3 0.3], 'LineWidth', 0.6);
% 
% legend([h_vlow, h_vhi, h_alias_d, h_alias_s, h_band_proxy], ...
%     {'Velocity SNR floor ($\delta/\lambda = 0.07$)', ...
%      'Linearity breakdown ($\delta/\lambda = 0.33$)', ...
%      'Diagonal Nyquist ($\delta/\lambda = 1/(2\sqrt{2})$)', ...
%      'Side-length Nyquist ($\delta/\lambda = 1/2$)', ...
%      'Aliasing-onset band'}, ...
...
function [tick_pos, tick_lab] = edge_ticks(test_values, edges)
    ...
    % edges that coincide with a test value (so the grid lines and
    % data labels stay aligned).
    tick_pos = edges;
    tick_lab = cell(1, length(edges));
    cell_spacing = median(diff(test_values));
    for i = 1:length(edges)
        [d, idx] = min(abs(test_values - edges(i)));
        if d < cell_spacing * 0.05
