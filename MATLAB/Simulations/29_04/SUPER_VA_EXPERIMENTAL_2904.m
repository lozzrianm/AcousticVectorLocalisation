...
% Frequency-locked bin selection and grid override by L Marshall 29/04/2026
%
% Changes (29/04/2026):
...
%     bandwidth is a narrow window around the test frequency where the
...
%   - 1D radial ray scans added to expose whether per-array responses
%     peak at the source distance or run off to the grid boundary
%     (the cardioid failure mode predicted by paper Eq. 16).
...
csv_filename_a1 = 'MPA416_MEMS_Array1_Signal.csv';
convert_wav_a1 = true;
...
csv_filename_a2 = 'MPA416_MEMS_Array2_Signal.csv';
convert_wav_a2 = true;
...
% Sign convention: positive offset = channel leads ch 1, correction lags
% Negative offset = channel lags ch 1, correction leads
...
% For MPA416 mics at 1 kHz this is well-supported by the measurement data.
ref_freq = 1000;
...
% TEMPORARY DIAGNOSTIC — A2 channel reorder for 90 deg rotation hypothesis
% Mapping: ch1 was in slot M4 (TL), ch2 in slot M2 (BR),
%          ch3 in slot M3 (TR), ch4 in slot M1 (BL)
% So in slot order [M1 M2 M3 M4] we want CSV columns [ch4 ch2 ch3 ch1]
% tx_a2 = tx_a2([4 2 3 1], :);

% MEMS PER-CHANNEL AMPLITUDE CALIBRATION %
% Linear gain multipliers from pairwise pistonphone Vpp measurements at 1 kHz
% (each pair recorded mic n alongside mic 1 simultaneously; ratio computed
% within each pair, then inverted to give the gain that brings mic n up to
% the mic 1 level).
% Source data:
...
% Applied identically to both arrays since the same physical mics are used.
...
%% 1D RAY SCANS - INDIVIDUAL ARRAYS %%
% Confirms whether each array's response peaks at the source distance
% or runs off to the grid boundary (a pure cardioid would do the latter
% regardless of range)

fprintf('\n1D Ray Scans - Individual Arrays:\n');
ray_results_arrays = cell(N_a, 1);
for array = 1:N_a
    array_centre_2d = mean(vs_centres_arrays{array}(1:2,:), 2);
    ray_results_arrays{array} = plot_1dray_scan_vs(...
        r_vs_arrays{array}, vs_centres_arrays{array}, ...
        array_centre_2d, source_positions(1, 1:2), ...
        bin_freqs, c_0, rho_0, loading, num_bins, N_v, ...
        sprintf('Array %d', array));
end
...
%% 1D RAY SCAN - SUPERIMPOSED %%

fprintf('\n1D Ray Scan - Superimposed:\n');
sys_centre = zeros(2, 1);
for array = 1:N_a
    sys_centre = sys_centre + mean(vs_centres_arrays{array}(1:2,:), 2);
end
sys_centre = sys_centre / N_a;
plot_1dray_scan_super(response_db_norm_combined, X_grid, Y_grid, ...
    sys_centre, source_positions(1, 1:2));
...
% at each bin — assumes a frequency-flat physical delay (cable/ADC group
% delay), which is reasonable for MPA416 mics at the calibration frequency.
...
% FUNCTION: PLOT_1DRAY_SCAN_VS
% 1D MVDR scan along the radial ray from a single AVS array centre through
% the true source position. A genuine range-resolving response peaks at the
% source distance; a pure cardioid response keeps growing toward the grid
% boundary because the cardioid has no range information.
function ray_result = plot_1dray_scan_vs(r_vs, vs_centres, array_centre_2d, ...
    source_2d, bin_freqs, c_0, rho_0, loading, num_bins, N_v, label)

    src_vec = source_2d(:) - array_centre_2d(:);
    src_dist = norm(src_vec);
    if src_dist < 1e-9
        warning('plot_1dray_scan_vs:source_at_array', ...
            'Source coincides with array centre — skipping ray scan.');
        ray_result = struct('peak_dist', NaN, 'peak_db', NaN, 'true_dist', src_dist);
        return;
    end
    ray_dir = src_vec / src_dist;

    % Sample 200 points along the ray, from 0.05 m out to 1.5x source dist
    num_points = 200;
    r_samples = linspace(0.05, 1.5 * src_dist, num_points);
    candidate_points = [array_centre_2d(1) + r_samples(:) * ray_dir(1), ...
                        array_centre_2d(2) + r_samples(:) * ray_dir(2), ...
                        zeros(num_points, 1)];

    response_db = mvdr_vs_beamforming(r_vs, vs_centres, candidate_points, ...
        bin_freqs, c_0, rho_0, loading, num_bins, N_v);

    [peak_db, peak_idx] = max(response_db);
    peak_dist = r_samples(peak_idx);

    at_boundary = (peak_idx == 1) || (peak_idx == num_points);
    range_error = peak_dist - src_dist;

    fprintf('  %s: peak %.2f dB at r = %.3f m (true %.3f m, err = %+.4f m)%s\n', ...
        label, peak_db, peak_dist, src_dist, range_error, ...
        ternary(at_boundary, ' [BOUNDARY]', ''));

    ray_result = struct('peak_dist', peak_dist, 'peak_db', peak_db, ...
        'true_dist', src_dist, 'range_error', range_error, ...
        'at_boundary', at_boundary, 'r_samples', r_samples, 'response_db', response_db);

    figure('Color', 'w', 'Position', [100 100 560 360]);
    plot(r_samples, response_db, 'b-', 'LineWidth', 1.8);
    hold on;
    xline(src_dist, 'r--', 'LineWidth', 1.5, 'DisplayName', 'True source');
    xline(peak_dist, 'k:', 'LineWidth', 1.2, 'DisplayName', 'MVDR peak');
    xlabel('Range along bearing ray (m)', 'FontName', 'Times New Roman', 'FontSize', 14);
    ylabel('Beamformer response (dB)', 'FontName', 'Times New Roman', 'FontSize', 14);
    title(sprintf('%s — 1D ray scan', label), ...
        'FontName', 'Times New Roman', 'FontSize', 13);
    legend({'Response', 'True source', 'MVDR peak'}, ...
        'Location', 'best', 'FontName', 'Times New Roman', 'FontSize', 10);
    grid on; box on;
    set(gca, 'FontName', 'Times New Roman', 'FontSize', 12);
end


% FUNCTION: PLOT_1DRAY_SCAN_SUPER
% 1D scan of the (already computed) superimposed response surface along
% the radial ray from the system midpoint through the true source. Does
% NOT recompute MVDR — samples the existing combined response surface.
function ray_result = plot_1dray_scan_super(response_db_combined, X_grid, Y_grid, ...
    system_centre_2d, source_2d)

    src_vec = source_2d(:) - system_centre_2d(:);
    src_dist = norm(src_vec);
    if src_dist < 1e-9
        warning('plot_1dray_scan_super:source_at_centre', ...
            'Source coincides with system centre — skipping ray scan.');
        ray_result = struct('peak_dist', NaN, 'peak_db', NaN, 'true_dist', src_dist);
        return;
    end
    ray_dir = src_vec / src_dist;
    ...
    num_points = 200;
    r_samples = linspace(0.05, 1.5 * src_dist, num_points);
    x_query = system_centre_2d(1) + r_samples(:) * ray_dir(1);
    y_query = system_centre_2d(2) + r_samples(:) * ray_dir(2);

    grid_response = reshape(response_db_combined, size(X_grid));
    response_db = interp2(X_grid, Y_grid, grid_response, x_query, y_query, 'linear', NaN);

    valid = ~isnan(response_db);
    if ~any(valid)
        warning('plot_1dray_scan_super:ray_outside_grid', ...
            'Ray falls entirely outside search grid — cannot scan.');
        ray_result = struct('peak_dist', NaN, 'peak_db', NaN, 'true_dist', src_dist);
        return;
    end

    [peak_db, peak_idx_local] = max(response_db(valid));
    valid_idx = find(valid);
    peak_idx = valid_idx(peak_idx_local);
    peak_dist = r_samples(peak_idx);

    at_boundary = (peak_idx == valid_idx(1)) || (peak_idx == valid_idx(end));
    range_error = peak_dist - src_dist;

    fprintf('  Superimposed: peak %.2f dB at r = %.3f m (true %.3f m, err = %+.4f m)%s\n', ...
        peak_db, peak_dist, src_dist, range_error, ...
        ternary(at_boundary, ' [BOUNDARY]', ''));
    ...
    figure('Color', 'w', 'Position', [100 100 560 360]);
    plot(r_samples, response_db, 'k-', 'LineWidth', 1.8);
    hold on;
    xline(src_dist, 'r--', 'LineWidth', 1.5, 'DisplayName', 'True source');
    xline(peak_dist, 'b:', 'LineWidth', 1.2, 'DisplayName', 'Peak');
    xlabel('Range along bearing ray (m)', 'FontName', 'Times New Roman', 'FontSize', 14);
    ylabel('Combined response (dB)', 'FontName', 'Times New Roman', 'FontSize', 14);
    title('Superimposed — 1D ray scan from system centre', ...
        'FontName', 'Times New Roman', 'FontSize', 13);
    legend({'Response', 'True source', 'Peak'}, ...
        'Location', 'best', 'FontName', 'Times New Roman', 'FontSize', 10);
    grid on; box on;
    set(gca, 'FontName', 'Times New Roman', 'FontSize', 12);
end
...
% FUNCTION: TERNARY
% Conditional value selection for inline use in fprintf etc.
