...
% Test C2 — superimposed dual-AVS vs ULA cross-architecture sweep
%
% Same structure as C1 but with the AVS side replaced by the two-AVS
% superimposed configuration (Array 1 at (0,0), Array 2 at (0,-0.32)).
% The dual-AVS aperture (~0.32 m) is comparable to the ULA aperture
% (0.36 m), so this is the geometrically fair comparison.
%
% AVS superposition follows your Test A AVA pipeline: per-array MVDR
% responses are mean-normalised, multiplied in linear power, then
% renormalised. Source position for the radial-error metric is taken
% from the ray-intersection between the per-array peak bearings, with
% the global peak as a fall-back (matching superimposed_localisation).
%
% Outputs:
%   c2_results.csv / .mat
%   c2_radial_crossover_<f>Hz.png/pdf
%   c2_angular_crossover_<f>Hz.png/pdf
%   c2_bracket_crossover_<f>Hz.png/pdf
%   c2_beamwidth_crossover_<f>Hz.png/pdf
%
% Same broadside half-plane restriction as C1 for the ULA peak search
...
% Dual-AVS configuration (matches Test A AVA script)
delta_avs = 0.04;
N_a = 2;
N_v = 1;
A1_centre = [0; 0; 0];
...
results_folder = sprintf('crossarch_testC2_superimposedvsULA_%s', timestamp);
mkdir(results_folder);

fprintf('\n<strong>TEST C2 — DUAL SUPERIMPOSED AVS vs ULA</strong>\n');
fprintf('Frequencies (%d): ', num_freq);   fprintf('%.0f  ', test_frequencies);   fprintf('Hz\n');
fprintf('r/lambda (%d): ', num_dist);      fprintf('%.2f  ', distance_lambdas); fprintf('\n');
fprintf('Bearings (%d): ', num_angle);     fprintf('%.0f  ', source_angles_deg); fprintf('deg\n');
fprintf('Total runs: %d\n', total_runs);
fprintf('AVS dual-array aperture: %.3f m | ULA aperture: %.3f m\n', ...
    norm(A2_centre - A1_centre), total_aperture_ula);
...
for fi = 1:num_freq
    ...
    for ri = 1:num_dist
        ...
        for ai = 1:num_angle
            ...
            fprintf('\n[%d/%d] f=%.0f Hz | r=%.2f lam | theta=%.0f deg | (%.3f, %.3f) m\n', ...
            ...
            % System centre = midpoint between AVS centres (for angular ref)
            ...
            % MVDR — ULA (with broadside half-plane restriction) %
            ...
            % Per-array peak bearings -> ray intersection -> superimposed
            % map peak as fall-back
            ...
            % Per-array peak bearings for the ray intersection
            ...
            % Build the superimposed map (mean-normalised product) for
            % bracket and angular metrics
            ...
            if isnan(est_x_avs)
                % Fall back to the global peak of the superimposed map
            ...
            % ULA: ray from ULA centre along true source bearing
            % AVS: ray from system centre along true bearing, beamformed
            %      with the superimposed (multiplied) MVDR response
            ...
            % Superimposed ray response — mean-normalised product of per-array responses
            ray_resp_super = zeros(ray_num_points, 1);
            ...
            % METRICS — beamwidth at source range (ULA only — superposition
            % is ill-defined for a circle around a single centre with two
            % AVS arrays at different positions; stored for reference) %
            ...
            fprintf('  Radial:   ULA=%.4f m | dual AVS=%.4f m\n', radial_err_ula, radial_err_avs);
            fprintf('  Angular:  ULA=%.3f deg | dual AVS=%.3f deg\n', ang_err_ula, ang_err_avs);
            fprintf('  Bracket:  ULA=%.4f (sat=%d) | dual AVS=%.4f (sat=%d)\n', ...
...
fprintf('\n<strong>C2 SWEEP COMPLETE in %.1f min</strong>\n', toc(total_tic) / 60);
...
%% LOCAL FUNCTIONS %%
% Identical to C1 — duplicated here so this script is self-contained.
% If you change anything, update both C1 and C2.
...
%     build_search_grid(test_freq, c_0, source_distance, centre_y, grid_pts_per_lambda, margin_fixed)
%     lambda = c_0 / test_freq;
%     grid_resolution = lambda / grid_pts_per_lambda;
%     max_extent = source_distance + margin_fixed;
%     nx = round(2 * max_extent / grid_resolution) + 1;
%     ny = round(2 * max_extent / grid_resolution) + 1;
