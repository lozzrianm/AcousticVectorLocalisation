...
N_v = 2; %No. of vector sensors per array
d_y = 0.1555; %Vector sensor y axis spacing (m)
...
grid_resolution = 0.015; %(m)
% Margin around source
margin = max(1.5 * lambda, 0.5);
max_extent = sqrt((A1_coord(1)-source_positions(1,1))^2+(A1_coord(2)-source_positions(1,2))^2) + margin;

% Calculate grid points for constant resolution
x_margin = 2 * max_extent; %(m)
y_margin = 2 * max_extent; %(m)
...
if N_a == 1
    x_centre = mean(vs_centres_arrays{1}(1, :));
    y_centre = mean(vs_centres_arrays{1}(2, :));
    % z_scan = 0; %2D plane, change if needed
else
    % Use array mean as grid centre
    % First find geom centre of each array
    x_array_centres = zeros(1, N_a);
    y_array_centres = zeros(1, N_a);

    for array = 1:N_a
        x_array_centres(array) = mean(vs_centres_arrays{array}(1, :));
        y_array_centres(array) = mean(vs_centres_arrays{array}(2, :));
    end

    % Then find system geometric centre
    x_centre = mean(x_array_centres);
    y_centre = mean(y_array_centres);
end

% Build scan grid around chosen centre
x_scan = linspace(x_centre - x_margin, x_centre + x_margin, x_scan_points);
y_scan = linspace(y_centre - y_margin, y_centre + y_margin, y_scan_points);
...
for array = 1:N_a
    ...
    [~, ~] = plot_2dscan_mvdr(r_vs_arrays{array}, vs_centres_arrays{array}, ...
        ...
        response_db_arrays{array}, array, grid_res);
end
...
if N_a == 1
    ...
    angular_refs = {mean(vs_centres_arrays{1}(1:2, :), 2)};
else
    ...
    % Calculate geometric centre of each array
...
if N_a > 1
% For superimposed response, create block-diagonal CSM
    fprintf('\nComputing beam pattern for superimposed arrays...\n');
    % Calculate geometric centre of each array first
    array_geometric_centres = zeros(2, N_a);
    for array = 1:N_a
        array_geometric_centres(:, array) = mean(vs_centres_arrays{array}(1:2, :), 2);
    end
    reference_centre = mean(array_geometric_centres, 2);  % Average across arrays

    % Create block-diagonal combined CSM
    csm_size = 3 * N_v;
    r_vs_combined = zeros(csm_size * N_a, csm_size * N_a, num_bins);
    for array = 1:N_a
        block_idx = (array-1)*csm_size + (1:csm_size);
        r_vs_combined(block_idx, block_idx, :) = r_vs_arrays{array};
    end
