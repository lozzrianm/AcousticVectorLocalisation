...
%this set grid res is just here for consistency with sims for comparative
%trials
source_frequency = 1000;
lambda = c_0/source_frequency;
margin = max(1.5 * lambda, 0.5);
max_extent = sqrt((A1_coord(1)-source_positions(1))^2+(A1_coord(2)-source_positions(2))^2) + margin;
...
apply_dc_removal = false; %remove DC offset from time-domain signals
...
for mic = 1:Nr
    ...
    % Sinusoidal signals
    sinusoidal_col = mic + 1 + Nr; %columns 13-23

    tx(:, mic) = data(:, monopole_col) + data(:, sinusoidal_col);
end

tx = tx.'; %transpose signals, rows = sensors, columns = time samples

%% SIGNAL CONDITIONING %%

if apply_dc_removal==true
...
function snapshots_corrected = apply_calibration_correction(snapshots, mic_sensitivity, phase_offset)  
    ...
    size_fft = size(snapshots, 2);
    num_snap = size(snapshots, 3);
    ...
    % The 1/sensitivity normalises amplitude
    % The exp(-i*phase_offset) removes the phase error
    correction_factors = (1 ./ mic_sensitivity) .* exp(-1i * phase_offset);

    % Reshape for broadcasting across all frequency bins and snapshots
    correction_factors = reshape(correction_factors, [N_mics, 1, 1]); 
    
    % Apply corrections: multiply each frequency bin by correction factor
