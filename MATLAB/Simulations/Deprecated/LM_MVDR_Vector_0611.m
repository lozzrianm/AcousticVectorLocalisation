...
x_a = 2.0 + delta/2; %Array geometric center x (m)
...
for vs = 1:N_v
    y_c_desired = -d_y/2;  
    vs_centres(:, vs) = [x_a + delta/2; y_c_desired + delta/2; z_a];
end
...
for vs = 1:N_v
    ...
    mic_positions(:,idx(1)) = [x_c - delta/2; y_c - delta/2; z_c]; % p0: bottom-left
    mic_positions(:,idx(2)) = [x_c + delta/2; y_c - delta/2; z_c]; % p1: bottom-right
    mic_positions(:,idx(3)) = [x_c + delta/2; y_c + delta/2; z_c]; % p2: top-right
    mic_positions(:,idx(4)) = [x_c - delta/2; y_c + delta/2; z_c]; % p3: top-left
end
