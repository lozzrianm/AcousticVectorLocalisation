...
% Sub-element trio spacing
delta = 0.012; %(m)

% Three MEMS microphones per sensor
N_v = 1; %Number of vector sensor elements
N_m = N_v*3; %Total MEMS microphones

%% Define microphone positions %%
mic_positions = zeros(3, N_m);

% Vector element positions
%y_centres = [-d_y/2, d_y/2];

for vs = 1:N_v
    idx = (vs-1)*3+(1:3); %MEMS mic indices
    x_c = x_a; %Same x pos for both
    %y_c = y_centres(vs); %Vector element
    y_c = 0;
    mic_positions(:,idx(1)) = [x_c; y_c; z_a]; %Centre
    mic_positions(:,idx(2)) = [x_c + delta; y_c; z_a]; %+x
...
fprintf('Microphones per AVS: 3 (for FD approximation)\n');
